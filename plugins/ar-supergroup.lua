...
local function lock_group_links(msg, data, target)
  ...
  if group_link_lock == 'yes' then
   return 'الروابط مقفوله بلفعل كبد عمري 🔐\n👮 الامر بواسطة :️ @'..msg.from.username
  else
    data[tostring(target)]['settings']['lock_link'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'تم قفل الروابط كبد عمري 🔐\n👮 الامر بواسطة :️ @'..msg.from.username
  end
end
...
local function unlock_group_links(msg, data, target)
  ...
  if group_link_lock == 'no' then
    return ' الروابط مفتوحه بلفعل كبد عمري  🔓\n👮 الامر بواسطة by :️ @'..msg.from.username
  else
    data[tostring(target)]['settings']['lock_link'] = 'no'
    save_data(_config.moderation.data, data)
    return 'تم فتح الروابط كبد عمري 🔓\n👮 الامر بواسطة :️ @'..msg.from.username
  end
end
...
local function lock_group_spam(msg, data, target)
  ...
  if group_spam_lock == 'yes' then
    return ' الكلايش مقفوله بلفعل 🔐\n👮 الامر بواسطة :️ @'..msg.from.username
  else
    data[tostring(target)]['settings']['lock_spam'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'تم قفل الكلايش كبد عمري  🔐\n👮 الامر بواسطة :️ @'..msg.from.username
  end
end
...
local function unlock_group_spam(msg, data, target)
  ...
  if group_spam_lock == 'no' then
    ...
    save_data(_config.moderation.data, data)
    return 'تم فتح الكلايش كبد عمري 🔓\n👮 الامر بواسطة :️ @'..msg.from.username
  end
end
...
local function lock_group_flood(msg, data, target)
  ...
  if group_flood_lock == 'yes' then
    ...
    save_data(_config.moderation.data, data)
    return 'تم قفل التكرار كبد روحي 🔓 🔐\n👮 الامر بواسطة :️ @'..msg.from.username
  end
end
...
local function unlock_group_flood(msg, data, target)
  ...
  if group_flood_lock == 'no' then
    ...
    save_data(_config.moderation.data, data)
    return 'تم فتح التكرار كبد روحي 🔓 🔓\n👮 الامر بواسطة :️ @'..msg.from.username
  end
end
...
local function unlock_group_arabic(msg, data, target)
  ...
  if group_arabic_lock == 'no' then
    ...
    save_data(_config.moderation.data, data)
    return 'تم فتح العربيه كبد روحي 🔓\n👮 الامر بواسطة :️ @'..msg.from.username
  end
end
...
local function lock_group_membermod(msg, data, target)
  ...
  if group_member_lock == 'yes' then
    ...
    return ' تم قفل الاضافه كبد روحي  🔐\n👮 الامر بواسطة :️ @'..msg.from.username
end
...
local function lock_group_rtl(msg, data, target)
  ...
  if group_rtl_lock == 'yes' then
    return ' الاضافه الجماعيه مقفوله بلفعل 🔐\n👮 الامر بواسطهy :️ @'..msg.from.username
  else
    data[tostring(target)]['settings']['lock_rtl'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'تم قفل الاضافه الجماعيه كبد روحي 🔐\n👮 الامر بواسطة :️ @'..msg.from.username
  end
end
...
local function unlock_group_rtl(msg, data, target)
  ...
  if group_rtl_lock == 'no' then
    return 'الاضافه الجماعيه مفتوحه بلفعل 🔓\n👮 الامر بواسطه :️ @'..msg.from.username
  else
    data[tostring(target)]['settings']['lock_rtl'] = 'no'
    save_data(_config.moderation.data, data)
    return 'تم فتح الاضافه الجماعيه كبد روحي 🔓\n👮 الامر بواسطه :️ @'..msg.from.username
  end
end
...
local function lock_group_sticker(msg, data, target)
  ...
  if group_sticker_lock == 'yes' then
    ...
    save_data(_config.moderation.data, data)
    return 'تم قفل الملصقات كبد  روحي 🔐\n👮 الامر بواسطة :️ @'..msg.from.username
  end
end
...
local function unlock_group_sticker(msg, data, target)
  ...
  if group_sticker_lock == 'no' then
    ...
    save_data(_config.moderation.data, data)
    return 'تم فتح الملصقات كبد عمري 🔓\n👮 الامر بواسطة :️ @'..msg.from.username
  end
end
...
local function enable_strict_rules(msg, data, target)
  ...
  if group_strict_lock == 'yes' then
    ...
    save_data(_config.moderation.data, data)
    return ' تم قفل التحذير كبد روحي   🛡\n👮 الامر بواسطة by :️ @'..msg.from.username
  end
end
...
local function disable_strict_rules(msg, data, target)
  ...
  if group_strict_lock == 'no' then
    return 'التحذير مفتوح بلفعل 🛡\n👮 الامر بواسطة :️ @'..msg.from.username
  else
    data[tostring(target)]['settings']['strict'] = 'no'
    save_data(_config.moderation.data, data)
    return 'تم فتح التحذير كبد روحي 🛡\n👮 الامر بواسطة :️ @'..msg.from.username
  end
end
...
local function callbackres(extra, success, result)
  ...
  elseif get_cmd == "invite" then
	...
	elseif get_cmd == 'mute_user' then
		...
		if is_muted_user(chat_id, user_id) then
			unmute_user(chat_id, user_id)
			send_large_msg(receiver, " ["..user_id.."] 👻 تم فتح الكتم عنك في🤖 هذا المجموعه😹 دردشه يلا يا ")
		elseif is_owner(extra.msg) then
			mute_user(chat_id, user_id)
			send_large_msg(receiver, " ["..user_id.."] 🤕تم كتمك في هذا المجموعه 👩‍👩‍👧‍👧لمخالفه القوانين🤖")
		end
	end
end
		...
		if matches[1] == 'قفل' and is_momod(msg) then
			...
			local msg_type = 'Documents'
				if not is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: mute "..msg_type)
					mute(chat_id, msg_type)
					return 'تم قفل الفايلات 📂🔐\n👮 الامر بواسطة :️ @'..msg.from.username
				else
					return 'الفايلات  مقفوله بلفعل 📂🔐\n👮 الامر بواسطة :️ @'..msg.from.username
				end
			end
          ...
          return 'تم فتح ارسال الفديو 🎥🔐\n👮 الامر بواسطة :️ @'..msg.from.username 
			...
			local msg_type = 'Documents'
				if is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: unmute "..msg_type)
					unmute(chat_id, msg_type)
					return 'تم فتح الفايلات 📂🔓\n👮 الامر بواسطة :️ @'..msg.from.username
				else
					return 'الفايلات غير مقفول 📂🔓\n👮 الامر بواسطة :️ @'..msg.from.username
				end
			end
			if matches[2] == 'الدردشه' then
			local msg_type = 'Text'
		...
		if matches[1] == "كتم" and is_momod(msg) then
			...
			elseif matches[1] == "كتم" and string.match(matches[2], '^%d+$') then
				...
				if is_muted_user(chat_id, user_id) then
					...
					return "["..user_id.."] 😉 جرَأرَهَ ❤️ رأَحَ أَلكـتَمَ 😄 منَكْ يـلآَ دردَشَ 💬"
				elseif is_momod(msg) then
					mute_user(chat_id, user_id)
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] added ["..user_id.."] to the muted users list")
					return "["..user_id.."] جَرأَرهَ ❤️ عمَيتَ علىَ نفسَكْ 😢 تمَ كـتَمَكَ 🤐"
				end
