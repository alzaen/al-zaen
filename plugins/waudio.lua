...
local monster = msg['id'] 
    chat_id =  msg.to.id 
    if matches[1] == 'lock' and matches[2] == "audio" and is_momod(msg) then 
                    local oscar = 'mate:'..msg.to.id 
                    redis:set(oscar, true) 
                    local dd = 'تم قفل بصمات مع تحذير {✔}😈' 
reply_msg(monster, dd, ok_cb, true) 
elseif matches[1] == 'lock' and matches[2] == 'audio' and not is_momod(msg) then 
local pxpp = 'للمشرفين فقط عزيزي👿' 
reply_msg(monster, pxpp, ok_cb, true) 

    elseif matches[1] == 'unlock' and matches[2] == 'audio' and is_momod(msg) then 
      local oscar = 'mate:'..msg.to.id 
      redis:del(oscar) 
    local gg = ' تم الغاء قفل بصمات مع تحذير {❎}😈' 
reply_msg(monster, gg, ok_cb, true) 
elseif matches[1] == 'unlock' and matches[2] == 'audio' and not is_momod(msg) then 
